...
/**
 * Bigbluebutton Import Model
 */
class BigbluebuttonModelImport extends JModelLegacy
{
	 ...
	 * @var        string
	 */
	protected $_context = 'com_bigbluebutton.import';
	...
	protected function populateState()
	{
		...
		$this->setState('message', $app->getUserState('com_bigbluebutton.message'));
	...
	public function import()
	{
		...
		if ($package === null)
		{
			switch ($this->getType)
			{
				...
				default:
					$app->setUserState('com_bigbluebutton.message', JText::_('COM_BIGBLUEBUTTON_IMPORT_NO_IMPORT_TYPE_FOUND'));
		...
		if (!$package || !$package['type'])
		{
			...
			$app->setUserState('com_bigbluebutton.message', JText::_('COM_BIGBLUEBUTTON_IMPORT_UNABLE_TO_FIND_IMPORT_PACKAGE'));
			return false;
		}
		...
		if (!$this->setData($package,$this->dataType,$headerList))
		{
			...
			$msg = JText::_('COM_BIGBLUEBUTTON_IMPORT_ERROR');
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
				$app->setUserState('com_bigbluebutton.redirect_url', 'index.php?option=com_bigbluebutton&view='.$back);
		...
		else
		{
			...
			$msg = JText::sprintf('COM_BIGBLUEBUTTON_IMPORT_SUCCESS', $package['packagename']);
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
			    $app->setUserState('com_bigbluebutton.redirect_url', 'index.php?option=com_bigbluebutton&view='.$back);
	...
	{		
		...
		{
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_WARNIMPORTFILE'), 'warning');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_NO_FILE_SELECTED'), 'warning');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_WARNIMPORTUPLOADERROR'), 'warning');
			return false;
		}
	...
	protected function _getPackageFromFolder()
	{
		...
		{
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_PLEASE_ENTER_A_PACKAGE_DIRECTORY'), 'warning');
			return false;
		}
		...
		if (!$type)
		{
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_PATH_DOES_NOT_HAVE_A_VALID_PACKAGE'), 'warning');
		}
		...
		{
			// set error message
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_DOES_NOT_HAVE_A_VALID_FILE_TYPE'), 'warning');
			return false;
		}
	...
	protected function _getPackageFromUrl()
	{
		...
		if (!$url)
		{
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_ENTER_A_URL'), 'warning');
			return false;
		}
		...
		if (!$p_file)
		{
			$app->enqueueMessage(JText::_('COM_BIGBLUEBUTTON_IMPORT_MSG_INVALID_URL'), 'warning');
			return false;
		}
	...
	**/
	...
	{
		if (BigbluebuttonHelper::checkArray($target_headers))
		{
	...
	**/
	protected function save($data,$table)
	{
		...
		if(BigbluebuttonHelper::checkArray($data['array']))
		{
			// get user object
			...
			if(BigbluebuttonHelper::checkArray($data['array']) && $user->authorise($table.'.import', 'com_bigbluebutton') && $user->authorise('core.import', 'com_bigbluebutton'))
			{
				// set target.
				...
				$canDo = BigbluebuttonHelper::getActions($table);
				...
				foreach($data['array'] as $row)
				{
					...
					if (isset($row[$id_key]) && is_numeric($row[$id_key]) && $row[$id_key] > 0)
					{
						...
						$query
							->select('version')
							->from($db->quoteName('#__bigbluebutton_'.$table))
					...
					if($found && $canEdit)
					{
						...
						$query->update($db->quoteName('#__bigbluebutton_'.$table))->set($fields)->where($conditions);
						$db->setQuery($query);
						$db->execute();
					}
					elseif ($canCreate)
					{
						...
						$query
							->insert($db->quoteName('#__bigbluebutton_'.$table))
						...
						if ($done)
						{
							...
							BigbluebuttonHelper::setAsset($aId,$table);
						}
					}
					else
					{
	...
	protected function getAliasesUsed($table)
	{
		...
		$columns = $db->getTableColumns('#__bigbluebutton_'.$table);
		if(isset($columns['alias'])){
			// Create a new query object.
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('alias')));
			$query->from($db->quoteName('#__bigbluebutton_'.$table));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
