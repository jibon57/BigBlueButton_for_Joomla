 ...
 */
class BigbluebuttonModelEvent extends JModelAdmin
{    
	/**
	 * @var        string    The prefix to use with controller messages.
	 * @since   1.6
	 */
	protected $text_prefix = 'COM_BIGBLUEBUTTON';
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_bigbluebutton.event';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'event', $prefix = 'BigbluebuttonTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_bigbluebutton.event');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_bigbluebutton.event', 'event', $options);
	...
	{
		return 'administrator/components/com_bigbluebutton/models/forms/event.js';
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__bigbluebutton_event'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_bigbluebutton.edit.event.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			{
				list($this->table->event_title, $this->table->alias) = $this->generateNewTitle($values['catid'], $this->table->alias, $this->table->event_title);
			}
			else
			{
				list($this->table->event_title, $this->table->alias) = $this->generateNewTitle($this->table->catid, $this->table->alias, $this->table->event_title);
			}
			...
			if (BigbluebuttonHelper::checkArray($uniqeFields))
			{
	...
	public function save($data)
	{
		...
		if($event_start > $event_end){
			JFactory::getApplication()->enqueueMessage(JText::_("COM_BIGBLUEBUTTON_ERROR_EVENT_DATES"), 'warning');
			return false;
		}
		...
		// Alter the event_title for save as copy
		if ($input->get('task') === 'save2copy')
		{
			$origTable = clone $this->getTable();
			$origTable->load($input->getInt('id'));

			if ($data['event_title'] == $origTable->event_title)
			{
				list($event_title, $alias) = $this->generateNewTitle($data['catid'], $data['alias'], $data['event_title']);
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$data['alias'] = JFilterOutput::stringURLUnicodeSlug($data['event_title']);
				}
				else
				{
					$data['alias'] = JFilterOutput::stringURLSafe($data['event_title']);
				}

				$table = JTable::getInstance('event', 'bigbluebuttonTable');

				if ($table->load(array('alias' => $data['alias'], 'catid' => $data['catid'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_BIGBLUEBUTTON_EVENT_SAVE_WARNING');
				}
	...
	protected function _generateNewTitle($alias, $title = null)
	{
		...
		while ($table->load(array('alias' => $alias)))
		{
			...
			if (BigbluebuttonHelper::checkArray($title))
			{
